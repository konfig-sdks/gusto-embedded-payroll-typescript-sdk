...
import { readableStreamToString, GustoEmbeddedPayrollError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new GustoEmbeddedPayrollError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new GustoEmbeddedPayrollError(e, e.response?.data)
                }
                if (innerError instanceof GustoEmbeddedPayrollError) {
                    // Got "GustoEmbeddedPayrollError" from the above try block
                    throw innerError;
                }
